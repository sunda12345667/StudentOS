...
export default function Reels() {
  ...
  const handleUpload = async () => {
    ...
    await base44.entities.Reel.create({
      ...
      author_role: 'student', likes: [], like_count: 0, comment_count: 0, view_count: 0,
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-black">Educational Reels</h1>
          <p className="text-muted-foreground mt-1">Short-form learning videos</p>
        </div>
        <Button className="gradient-brand border-0 gap-2" onClick={() => setOpen(true)}>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {reels.map((reel, i) => (
            <ReelCard key={reel.id} reel={reel} user={user} onLike={handleLike} index={i} />
          ))}
        </div>
      )}
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 mt-2">
            <div>
              ...
              <div className="mt-1 border-2 border-dashed border-border rounded-xl p-6 text-center cursor-pointer hover:border-primary transition-colors" onClick={() => document.getElementById('reel-video').click()}>
                {videoFile ? (
...
function ReelCard({ reel, user, onLike, index }) {
      ...
      <Card className="overflow-hidden hover:shadow-xl transition-all group">
        <div className="relative aspect-[9/16] bg-black cursor-pointer" onClick={togglePlay}>
          ...
          <button
            ...
            className="absolute top-3 right-3 p-1.5 rounded-full bg-black/40 text-white opacity-0 group-hover:opacity-100 transition-opacity"
          >
          ...
          <div className="absolute right-3 bottom-3 flex flex-col gap-3">
            <button
              onClick={e => { e.stopPropagation(); onLike(reel); }}
              className="flex flex-col items-center gap-0.5"
            >
              <div className={`w-9 h-9 rounded-full flex items-center justify-center shadow-lg transition-all ${liked ? 'bg-red-500' : 'bg-black/50'}`}>
            ...
            <div className="flex flex-col items-center gap-0.5">
              <div className="w-9 h-9 rounded-full bg-black/50 flex items-center justify-center shadow-lg">
