...
import { Send, Bot, Sparkles, Loader2, RefreshCw, BookOpen, Calculator, FlaskConical, Globe, Code2, Layers, HelpCircle, FileText, Target, GraduationCap, Compass, Wand2, ChevronDown, PanelLeftOpen, PanelLeftClose } from 'lucide-react';
...
export default function AITutor() {
  ...
  useEffect(() => {
    ...
    base44.entities.TutorConversation.filter({ user_email: user.email }, '-last_message_at', 50)
      .then(convos => {
        setConversations(convos);
        // Auto-load the most recent non-archived conversation
        const recent = convos.find(c => !c.archived);
        if (recent) {
          setActiveConvId(recent.id);
          setMessages(recent.messages || []);
          setLevel(recent.level || 'secondary');
        }
      })
      .finally(() => setLoadingConvos(false));
  }, [user?.email]);

  const persistMessages = async (msgs, convId, title) => {
    if (!user?.email) return;
    setSaving(true);
    const data = {
      messages: msgs,
    ...
    if (convId) {
      await base44.entities.TutorConversation.update(convId, data);
      setConversations(prev => prev.map(c => c.id === convId ? { ...c, ...data } : c));
    } else {
      // Create new conversation
      const newTitle = title || 'New Conversation';
      const created = await base44.entities.TutorConversation.create({
  ...
  const sendMessage = async (text) => {
    ...
    const title = isFirst ? generateTitle(userText) : undefined;
    if (isFirst && activeConvId) {
      // Update existing convo title too
      await base44.entities.TutorConversation.update(activeConvId, { title });
      setConversations(prev => prev.map(c => c.id === activeConvId ? { ...c, title } : c));
    }
    await persistMessages(finalMessages, activeConvId, title);
  };
  ...
  const handleSelectConversation = (conv) => {
    ...
    setLevel(conv.level || 'secondary');
  };

  const handleDeleteConversation = async (convId) => {
    if (!window.confirm('Delete this conversation?')) return;
    await base44.entities.TutorConversation.delete(convId);
    setConversations(prev => prev.filter(c => c.id !== convId));
    if (activeConvId === convId) { setActiveConvId(null); setMessages([]); }
  };

  const handleArchiveConversation = async (convId) => {
    await base44.entities.TutorConversation.update(convId, { archived: true });
    setConversations(prev => prev.map(c => c.id === convId ? { ...c, archived: true } : c));
    if (activeConvId === convId) { setActiveConvId(null); setMessages([]); }
  };

  const handleRenameConversation = async (convId, newTitle) => {
    await base44.entities.TutorConversation.update(convId, { title: newTitle });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      {/* AI Chat Tab — with sidebar */}
      {activeTab === 'chat' && (
        <div className="h-[calc(100vh-22rem)] flex gap-0 rounded-2xl border border-border overflow-hidden">
          {/* Sidebar */}
          <AnimatePresence>
            {sidebarOpen && (
              <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 220, opacity: 1 }} exit={{ width: 0, opacity: 0 }}
                transition={{ duration: 0.2 }} className="flex-shrink-0 overflow-hidden" style={{ minWidth: sidebarOpen ? 220 : 0 }}>
                <TutorSidebar
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-4 py-2 border-b border-border bg-card">
              ...
              {messages.length > 0 && (
                <Button variant="ghost" size="sm" className="h-7 text-xs gap-1.5 text-muted-foreground" onClick={handleNewConversation}>
            ...
            <div className="border-t p-4">
              <form onSubmit={e => { e.preventDefault(); sendMessage(); }} className="flex gap-3">
                <Input value={input} onChange={e => setInput(e.target.value)} placeholder="Ask EduBot anything..."
                  className="flex-1 rounded-full" disabled={loading} />
