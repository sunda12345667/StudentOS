...
import {
  ...
  ShieldAlert, CheckCircle2, FileText, ShieldCheck
} from 'lucide-react';
...
export default function AdminUsers() {
  ...
  useEffect(() => {
    ...
    base44.entities.VerificationRequest.filter({ status: 'pending' })
      .then(reqs => setPendingVerifCount(reqs.length))
      .catch(() => {});
  }, []);
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${tab === 'users' ? 'bg-primary text-white' : 'bg-white/5 text-white/40 hover:text-white/70'}`}
        >
          <Users className="w-4 h-4" /> Users ({users.length})
        </button>
        <button
          onClick={() => setTab('verifications')}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${tab === 'verifications' ? 'bg-primary text-white' : 'bg-white/5 text-white/40 hover:text-white/70'}`}
        >
          <ShieldCheck className="w-4 h-4" /> Verifications
          {pendingVerifCount > 0 && (
            <span className="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{pendingVerifCount}</span>
          )}
        </button>
      </div>

      {/* Verifications tab */}
      {tab === 'verifications' && <VerificationRequests />}
